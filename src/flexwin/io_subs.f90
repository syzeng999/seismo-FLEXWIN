...
! -----------------------------------------------------------------------
  subroutine write_mt_input_3(WIN_IO,file_o,file_s)
    use seismo_variables
    implicit none
    ...
    if(num_win > 0) then
       ...
       write(WIN_IO,'(a)') trim(file_o)
       write(WIN_IO,'(a)') trim(file_s)
       write(WIN_IO,*) num_win
       do i = 1, num_win
          write(WIN_IO,'(2(f10.4,1x))') win_start(i), win_end(i)
       enddo

    endif

  end subroutine write_mt_input_3

!-----------------------------------------------------------------------

  subroutine get_dir(dir,basename)
    ...
    write(*,*)trim(dir)
    write(*,*)trim(basename)
    call find_last_char(loc,basename,'/')
...
!-----------------------------------------------------------------------
  ...
  subroutine find_last_char(loc,str,split_char)

    implicit none

    character*120 :: str,temp_str
    integer :: loc,loc_temp
    character :: split_char

    loc=0
    temp_str=str
    !write(*,*)"in find_last_char:  split_char=",split_char
    !write(*,*)""
    loc_temp=index(temp_str,split_char)
    !write(*,*)"loc_temp: ",loc_temp
    do while (loc_temp.ne.0)
      loc=loc+loc_temp
      !write(*,*)"loc: ",loc
      temp_str=str((loc+1):len(str))
      !write(*,*)"temp_str:",trim(temp_str)
      loc_temp=index(temp_str,split_char)
      !write(*,*)"loc_temp:",loc_temp
    end do

  end subroutine find_last_char
