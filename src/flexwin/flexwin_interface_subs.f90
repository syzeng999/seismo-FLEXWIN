...
subroutine read_main_parfile_mpi(rank, comm, ierr)
  ...
  call MPI_Bcast(PERIOD_BEGIN, 1, MPI_DOUBLE_PRECISION, 0, comm, ierr)
  call MPI_Bcast(PERIOD_END, 1, MPI_DOUBLE_PRECISION, 0, comm, ierr)

  call MPI_Bcast(WRITE_SINGLE_WIN_FILE, 1, MPI_LOGICAL, 0, comm, ierr)
  call MPI_Bcast(REMOVE_SURFACE_WAVE, 1, MPI_LOGICAL, 0, comm, ierr)
...
subroutine read_main_parfile(ierr)
  ...
  read(IIN, *)
  read(IIN, *)
  read(IIN,5) dummy_string, PERIOD_BEGIN
  read(IIN,5) dummy_string, PERIOD_END
  print *, "FLEXWIN PERIOD: ", PERIOD_BEGIN, PERIOD_END

  read(IIN, *)
  read(IIN, *)
  read(IIN, 3) dummy_string, WRITE_SINGLE_WIN_FILE
  read(IIN, 3) dummy_string, REMOVE_SURFACE_WAVE
  print *, "WRITE_SINGLE_WIN_FILE: ", WRITE_SINGLE_WIN_FILE
  print *, "REMOVE_SURFACE_WAVE: ", REMOVE_SURFACE_WAVE
