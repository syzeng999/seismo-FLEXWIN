...
!! Group Member: Wenjie Lei(lei@princeton.edu), Ebru Bozdag(bozdag@princeton.edu), James A. Smith(jas11@princeton.edu)
!! Bug Report: lei@princeton.edu
module fw_interface

contains

!> flexwin interface called by the WORKFLOW
!! @param obsd_all observed data(asdf container) -- input
!! @param synt_all synthetic data(asdf container) -- input
!! @param win_all window data -- output
!! @param rank MPI environment varialbes(EV) -- input
!! @param nproc MPI EV -- input
!! @param comm MPI EV -- input
!! @param ierr error code -- output
subroutine flexwin_interface(obsd_all, synt_all, win_all, &
  ...
  !-------------------------------------------------------------------.
  !read parfile:  the parameter depends on the frequency, components,
  ! and event depth. Since one asdf file only contains one earthquake 
  ! at a certain frequency band, we read three components parameter
  ! settings at the very beginning.
  !-------------------------------------------------------------------'
  ...
  !--------------------------------------------------------------------.
  !FLEXWIN subroutine: it takes one observed and one synthetic data,
  ! and also a lot of other informations, like receiver location, event
  ! information and so.
  !--------------------------------------------------------------------'
  do i=1, obsd_all%nrecords
     call flexwin(obsd_all%records(i)%record,obsd_all%npoints(i),obsd_all%sample_rate(i),obsd_all%begin_value(i),&
      ...
      synt_all%event, flexwin_par_all, win_all(i) )
  enddo

  call MPI_Barrier(comm, ierr)

  !--------------------------------------------.
  !write out window info as one large file
  !--------------------------------------------'
  ...
  call win_write_mt(FLEXWIN_OUTDIR, obsd_all%event, obsd_all%min_period,&
        ...
        win_all, rank)
  !---------------------------------------------------------------------.
  !write out window info as many individual files(one records per file) 
  !---------------------------------------------------------------------'
