...
# VERTICAL COMPONENT
	set z = `ls "/tigress-hsm/lei/9s-seismograms/OBSD_final/"$cmt"_"$seis"/CMT/select"/*LHZ* | wc -l` 
	echo $z
	echo $z > /tigress-hsm/lei/SOURCE/input_files/$cmt"/input_cmt_LHZ_"$seis
	foreach i ("/tigress-hsm/lei/9s-seismograms/OBSD_final/"$cmt"_"$seis"/CMT/select/"*LHZ*)
	set sta = `echo $i | awk 'BEGIN { FS = "select/" } ; { print $2 }' | awk -F. '{print ($1)}'`
   	...
   	set a = "/tigress-hsm/lei/9s-seismograms/SYNT_final/"$cmt"_"$seis"/CMT/select/"$sta"."$nw"."$cmp".sem"
     # echo $a
   	set b = "/tigress-hsm/lei/SOURCE/input_files/"$cmt"/MEASURE_cmt_"$cmp"_"$seis"/"$sta"."$nw"."$cmp
...
# RADIAL COMPONENT
	set z = `ls "/tigress-hsm/lei/9s-seismograms/OBSD_final/"$cmt"_"$seis"/CMT/select"/*LHR* | wc -l`
	echo $z
	echo $z > /tigress-hsm/lei/SOURCE/input_files/$cmt"/input_cmt_LHR_"$seis

	foreach i ("/tigress-hsm/lei/9s-seismograms/OBSD_final/"$cmt"_"$seis"/CMT/select/"*LHR*)
		set sta = `echo $i | awk 'BEGIN { FS = "select/" } ; { print $2 }' | awk -F. '{print ($1)}'`
	#	echo $sta
   	set nw = `echo $i | awk -F. '{print ($2)}'`
   	#set nm = `echo $i | awk -F. '{print ($3)}'`
...
# TRANSVERSE COMPONENT
	...
	set z = `ls "/tigress-hsm/lei/9s-seismograms/OBSD_final/"$cmt"_"$seis"/CMT/select"/*LHT* | wc -l`
      echo $z
	echo $z > /tigress-hsm/lei/SOURCE/input_files/$cmt"/input_cmt_LHT_"$seis

	foreach i ("/tigress-hsm/lei/9s-seismograms/OBSD_final/"$cmt"_"$seis"/CMT/select/"*LHT*)
   	set sta = `echo $i | awk 'BEGIN { FS = "select/" } ; { print $2 }' | awk -F. '{print ($1)}'`
   	set nw = `echo $i | awk -F. '{print ($2)}'`
   	#set nm = `echo $i | awk -F. '{print ($3)}'`
   	set cmp = `echo $i | awk -F. '{print ($3)}'`
