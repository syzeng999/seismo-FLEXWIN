...
subroutine set_up_criteria_arrays
  ...
  !****************************************
  !for test: just set default and return
  !call set_up_criteria_arrays_test
  !print *, "set up test"
  !return
  !print *, "no here"
  !****************************************
...
! -----------------------------------------------------------------
  ...
  if (DATA_QUALITY) then
    ...
    noise_start=b
    noise_end=max(ph_times(1)-WIN_MAX_PERIOD, b+dt)
    signal_start = noise_end
...
! -------------------------------------------------------------
  ...
  subroutine set_up_criteria_arrays_test
  use seismo_variables 
  implicit none
